...
setup() {
    ...
    printf 'github "%s/%s" ~> 1.8' "remirobert" "${PROJECT_NAME}" > Cartfile
...
@test "Carthage builds ${PROJECT_NAME}" {
    ...
    if [ -n "${skip_Dotzu}" ]; then
        skip "the variable skip_Dotzu is not empy: ${skip_Dotzu}"
    fi
